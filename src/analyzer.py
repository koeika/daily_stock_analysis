...
- **RSI超买超卖判断**：
  ...
  - RSI > 70：超买，回调风险高
  - 50 < RSI < 70：强势健康
  - RSI(12)为主要参考，结合RSI(6)和RSI(24)综合判断
- **KDJ指标（A股核心）**：
  - 低位金叉(K上穿D且K<30)：强烈买入信号
  - 高位死叉(K下穿D)：卖出信号
  - K/D/J>80：超买；K/D/J<20：超卖
- **BOLL布林带**：
  - 价格跌破下轨：超卖，关注反弹
  - 价格接近下轨：潜在买点
  - 价格在中轨获得支撑：偏多
...
### 强烈买入（80-100分）：
- ✅ 多头排列：MA5 > MA10 > MA20 (28分)
- ✅ 低乖离率：<2%，最佳买点 (18分)
- ✅ 缩量回调或放量突破 (12分)
- ✅ 筹码集中健康 (额外加分)
- ✅ **MACD零轴上金叉或金叉** (12分)
- ✅ **RSI超卖(<30)或强势(50-70)** (7分)
- ✅ **KDJ低位金叉或超卖反弹** (8分)
- ✅ **BOLL下轨支撑或超卖反弹** (7分)
- ✅ 消息面有利好催化 (额外加分)
...
💡 {'今日成交量为零，数据异常或为非交易日，无法判断真实量能。需结合后续交易日确认。' if (today.get('volume') in [None, 0] or today.get('amount') in [None, 0]) else ''}
...
| 指标 | 数值 | 说明 |
|------|------|------|
| DIF (快线) | {trend.get('macd_dif', 0):.4f} | 快速EMA(12)-慢速EMA(26) |
| DEA (慢线) | {trend.get('macd_dea', 0):.4f} | DIF的9日EMA |
| MACD柱 | {trend.get('macd_bar', 0):.4f} | (DIF-DEA)×2，红柱/绿柱 |
| MACD状态 | {trend.get('macd_status', '未知')} | |
| **信号解读** | **{trend.get('macd_signal', '无信号')}** | ⭐零轴上金叉最强/✅金叉/❌死叉 |

### RSI指标（超买超卖判断）
| 指标 | 数值 | 健康范围 |
|------|------|----------|
| RSI(6) 短期 | {trend.get('rsi_6', 50):.1f} | 30-70为正常 |
| RSI(12) 中期 | {trend.get('rsi_12', 50):.1f} | 主要参考指标 |
| RSI(24) 长期 | {trend.get('rsi_24', 50):.1f} | 长期趋势 |
| RSI状态 | {trend.get('rsi_status', '中性')} | |
| **信号解读** | **{trend.get('rsi_signal', '无信号')}** | ⭐超卖(<30)反弹机会/⚠️超买(>70)回调风险 |

### KDJ指标（A股核心随机指标）
| 指标 | 数值 | 说明 |
|------|------|------|
| K 值 | {trend.get('kdj_k', 50):.1f} | 快线 |
| D 值 | {trend.get('kdj_d', 50):.1f} | 慢线 |
| J 值 | {trend.get('kdj_j', 50):.1f} | 敏感线 |
| KDJ状态 | {trend.get('kdj_status', '中性')} | |
| **信号解读** | **{trend.get('kdj_signal', '无信号')}** | ✅低位金叉买入/❌死叉卖出/⭐超卖反弹 |

### BOLL布林带
| 指标 | 数值 | 说明 |
|------|------|------|
| 上轨 | {trend.get('boll_upper', 0):.2f} | 压力位 |
| 中轨 | {trend.get('boll_mid', 0):.2f} | MA20 |
| 下轨 | {trend.get('boll_lower', 0):.2f} | 支撑位 |
| 通道位置 | {trend.get('boll_position', 0):.2f} | -1(下轨)~1(上轨) |
| BOLL状态 | {trend.get('boll_status', '通道内')} | |
| **信号解读** | **{trend.get('boll_signal', '无信号')}** | ⭐跌破下轨超卖/✓中轨支撑 |
